...
const App = () => {
  ...
  const fetchWeatherData = async (url, name = '') => {
    ...
    try {
      ...
      if (!response.ok) throw new Error((await response.json()).message || 'City & Country not found')
 ...
 <div className='flex item-center justify-center p-6 min-h-screen'>
  ...
  <h1 className='text-4xl font-extrabold text-center mb-6'>
    weather App
      ...
      {weather ? (
        <form onSumbit={handleSearch} className='flex flex-col relative'>
          ...
          {suggestion.length > 0 && (
            <div className='absolute top-12 left-0 right-0 bg-transparent shadow-md rounded z-10'>
             ...
             <button type='sumbit' className='bg-purple-700 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded transition-colors'>
               Get Weather
             </button>
        </form>
      ):(
        <div className='mt-6 text-center transition-opacity duration-500'>
          <button onClick={() => { setWeather(null); setCity('') }}
            className='mb-4 bg-purple-600 hover:bg-blue-700 text-whitw font-semibold py-1 px-3 rounded transition-colors'>
              New Search
            </button>
            ...
            <div className='flex justify-between items-center'>
              ...
              <img src={`https://api.opemweathermap.org/img/wn/${weather.weather[0].icon}@2px.png`} alt={weather.weather[0].description}
